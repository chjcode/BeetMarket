...
//    LikeInfoDto fetchLikeInfo(Long postId, Long userId);
    ...
    @Query(value = "SELECT * FROM favorite f WHERE f.post_id = :postId AND f.user_id = :userId",
            nativeQuery = true)
    Optional<Favorite> findByPostIdAndUserId(@Param("postId") Long postId, @Param("userId") Long userId);
}
